      ...
      <div><h2>Past orders <span className="count">{orders.length}{orders.length === 50 ? "+" : ""}</span></h2>
        {orders.length === 0 ? <div className="admin-placeholder"><p>No orders have been placed yet.</p></div> : <div className="order-list">
          {orders.map((order) => <article className="order-card" key={order.id}>
            ...
            <ul className="order-items">{order.items.map((item) => <li key={item.id}><span>{item.quantity}× {item.drinkName}<small>{item.addons.map((addon) => addon.addonName).join(", ") || "No add-ons"}</small></span><strong>{formatMoney(item.unitPriceCents * item.quantity)}</strong></li>)}</ul>
          </article>)}
        </div>}
      </div>
    </section>
